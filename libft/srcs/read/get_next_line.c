...
/**
 * @brief Finds the next newline character in the buffer
 * 
 * @param fd		The file descriptor to read from.
 * @param buffer	The buffer to search in.
 * @param line		The line buffer.
 * @param newline	The newline character.
 * @return char*	The pointer to the newline character, NULL if not found.
 */
...
/**
 * @brief Get the next line of a file based on the file descriptor given
 * 
 * @param fd The file descriptor to read from.
 * @return char* The next line, NULL if an error occurs or EOF is reached.
 */
char	*get_next_line(int fd)
{
	...
	if (newline)
	{
		...
		tmp = line;
		line = ft_strndup(line, (size_t)(newline - line + 1));
		free(tmp);
	}
	else
		buffer[fd][0] = '\0';
